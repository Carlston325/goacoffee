import React from "react";

function SustainCoffee() {
  return (
    <>
      <main className="epUgWD" style={{ minHeight: "400px" }}>
        <article className="bsVIun">
          <div className="jEobIm">
            <h2>Sourcing our coffee beans sustainably</h2>
            <p>
              Coffee is at the absolute core of our business and we believe that
              great coffee has the power to create positive change. We make
              careful choices about the coffee we buy, working with the
              Rainforest Alliance to source our famous beans to their high
              standard.
              <br />
            </p>
          </div>
        </article>

        <article className="egFTCM">
          <div>
            <h3 className="article_title">Our progress and promises</h3>
          </div>
          <div className="iLfAmY">
            <div className="kbNriO">
              <div className="gpBKZs card ">
                <div className="dOxNqn">
                  <h4>Now</h4>
                  <p>
                    100% of our coffee beans in the UK are Rainforest Alliance
                    Certified.
                  </p>
                </div>
              </div>
              <div className="gpBKZs card">
                <div className="dOxNqn">
                  <h4>Near</h4>
                  <p>
                    We’ll collaborate with coffee experts to run projects
                    focused on implementing more sustainable coffee farming
                    methods.
                  </p>
                  <p></p>
                </div>
              </div>
              <div className="gpBKZs card">
                <div className="dOxNqn">
                  <h4>Next</h4>
                  <p>
                    We’ll assess the impact our business has on nature, and set
                    specific targets to reduce it.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </article>

        <article className="bsVIun">
          <div className="ldNozT">
            <h2>Our work with the Rainforest Alliance&nbsp;</h2>
            <p>
              We ensure that 100% of our coffee beans come from Rainforest
              Alliance Certified farms. This means that for every cup of coffee
              we serve, we invest back into the coffee-growing communities we
              depend on.&nbsp;
            </p>
            <p>
              At present, due to global supply chain disruptions, we are
              temporarily unable to source enough certified coffee with the
              Rainforest Alliance seal to maintain 100% coverage in our stores,
              Costa Express, and Ready to Drink and At Home ranges. These
              products continue to contain at least 70% Rainforest Alliance
              Certified coffee ingredients.
              <br />
              <br />
              We'll return to sourcing all our coffee from Rainforest Alliance
              Certified farms as soon as these disruptions ease. Find out more
              <a href="https://www.rainforest-alliance.org/business/certification/coffee-supply-chain-disruptions/">
                here
              </a>
              .
            </p>
            <h3>A better future for nature, people and businesses</h3>
            <p>
              <div className="inline-right coffee_source_image"></div>
              The{" "}
              <a href="https://www.rainforest-alliance.org">
                Rainforest Alliance
              </a>{" "}
              is an international non-profit organisation, working to create a
              better future for people and nature by making responsible business
              the new normal. By working together with farmers, businesses and
              consumers, its aim is to protect forests, improve the livelihoods
              of farmers and farming communities, and help them adapt to climate
              change.
            </p>
            <p>
              The Rainforest Alliance, and its little green frog seal, is
              recognised by companies and consumers around the world as a symbol
              of environmental, social and economic sustainability.
            </p>
            <p>
              Rainforest Alliance certification makes a huge impact across a
              whole range of different areas. These can be split up into three
              sections: <strong>people</strong>, <strong>planet</strong> and{" "}
              <strong>profit</strong>.
            </p>
          </div>
        </article>

        <article id="whats-next" className="dfSiCt">
          <div className="dhXnTr">
            <div className="eeECea">
              <h3>Cups and packaging</h3>
              <p>
                We're creating, choosing and championing more sustainable cups
                and packaging solutions, promoting recycling and driving reuse
                wherever we can.
              </p>
            </div>
            <ul className="gsgHhh">
              <li>
                <span className="jkcThw linkCta">
                  <a
                    className="link btnDark"
                    href="/sustainability/cups-and-packaging"
                  >
                    Read more
                  </a>
                </span>
              </li>
              <li>
                <span className="kuucrm linkCta">
                  <a className="link btnDark" href="/sustainability">
                    Back to Sustainability
                  </a>
                </span>
              </li>
            </ul>
          </div>
          <div className="kOtuDH">
            <div
              className="image_container image_container-constrained"
              style={{ height: "100%", minHeight: "300px", width: "100%" }}
            >
              <div className="article_bottom_image--cups_n_packages"></div>
            </div>
          </div>
        </article>
      </main>
    </>
  );
}

export default SustainCoffee;
