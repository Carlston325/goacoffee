function ClickAndCollect() {
  return (
    <>
      <main className="epUgWD" style={{ minHeight: "400px" }}>
        <article className="bsVIun">
          <div className="ldNozT">
            <h3>Take us with you</h3>
            <p>
              <span className="clear"></span>
              <img
                alt="Carlstons Club app showing the collect menu on a phone screen"
                src="//images.ctfassets.net/royi30b2qd26/6L1bk3dfopXmBQ7oNfxmvu/8cfec31b6b29b2c1441ffa1d2122e694/costa-club-june-23-click-collect-389x280.jpg"
                className="inline-right"
              />
              It's never been simpler to get your favourite Carlstons Coffee on
              the go.
            </p>
            <p>
              Download the{" "}
              <a href="https://swf3j.app.goo.gl/app">Carlstons Coffee app</a>{" "}
              and choose from a selection of our most popular food and drink
              items from our menu. Once you've picked your favourites and
              selected your nearest store, your order will be freshly prepared
              in-store ready for you to pick up and takeaway.
            </p>
            <p>
              And the best bit? There's no need to queue. When it's ready, our
              baristas will hand you your order from the pick-up point. Take a
              look below for details on how to order and don't forget to check
              which stores you can collect from using our{" "}
              <a href="https://www.costa.co.uk/locations/store-locator">
                store locator
              </a>
              .
            </p>
          </div>
        </article>

        <article id="app-promo" className="fbmffN">
          <div className="gIDlBP">
            <div className="hRutP">
              <h3>
                Skip the queue. <br />
                Tap the app.
              </h3>
              <p>
                Order ahead with the Carlstons Coffee app to collect your
                favourites in store. And the best thing? All of your Carlstons
                Club details are in the palm of your hand, so there’s no need to
                carry your loyalty card. Download our app to get started.
              </p>
            </div>
            <ul className="enhTis">
              <li className="bIrHbT">
                <a
                  href="https://apps.apple.com/app/id578627826"
                  className="hucpiK"
                >
                  <img
                    src="//images.ctfassets.net/royi30b2qd26/6eYanDnLtkVbCiZnjOc8aY/5eff88f78a85eefe8535d8940d7c401e/apple-cta-mobile_3x.png"
                    alt="IOS"
                    className="hYcCuN"
                  />
                </a>
              </li>
              <li className="bIrHbT">
                <a
                  href="https://play.google.com/store/apps/details?id=uk.co.club.costa.costa"
                  className="hucpiK"
                >
                  <img
                    src="//images.ctfassets.net/royi30b2qd26/JLx2EQieRalp2GUzG9W5h/1b3380a59dc7eb61414660ac9cb85ddc/google-cta-mobile_3x.png"
                    alt="Android"
                    className="hYcCuN"
                  />
                </a>
              </li>
            </ul>
          </div>
          <div className="gVWkJc">
            <div
              className="image_container image_container-constrained"
              style={{ height: "100%" }}
            >
              <div style={{ maxWidth: "700px", display: "block" }}>
                <img
                  alt=""
                  src="data:image/svg+xml;charset=utf-8,%3Csvg%20height='515'%20width='700'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E"
                  style={{
                    maxWidth: "100%",
                    display: "block",
                    position: "static",
                  }}
                />
              </div>
              <img
                style={{
                  opacity: "0",
                  transition: "opacity 500ms linear",
                  objectFit: "cover",
                }}
                src="data:image/jpeg;base64,"
                alt=""
              />
              <picture>
                <source
                  type="image/webp"
                  srcSet="
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=174&amp;h=128&amp;q=100&amp;fm=webp 174w,
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=348&amp;h=256&amp;q=100&amp;fm=webp 348w,
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=696&amp;h=512&amp;q=100&amp;fm=webp 696w
                        "
                  sizes="(min-width: 696px) 696px, 100vw"
                />
                <img
                  width="700"
                  height="515"
                  data-main-image=""
                  style={{ objectFit: "cover", opacity: "1" }}
                  sizes="(min-width: 696px) 696px, 100vw"
                  decoding="async"
                  loading="lazy"
                  src="https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=696&amp;h=512&amp;fl=progressive&amp;q=100&amp;fm=jpg"
                  srcSet="
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=174&amp;h=128&amp;fl=progressive&amp;q=100&amp;fm=jpg 174w,
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=348&amp;h=256&amp;fl=progressive&amp;q=100&amp;fm=jpg 348w,
                            https://images.ctfassets.net/royi30b2qd26/1tZcH146I4IzcGORgRh1TT/5887b58aa00e0481f25f1b65f940f0d2/App_Screen_Lifestyle_Free_Drink_June23.jpg?w=696&amp;h=512&amp;fl=progressive&amp;q=100&amp;fm=jpg 696w
                        "
                  alt="Carlstons Club app screen"
                />
              </picture>
            </div>
          </div>
        </article>

        <article className="gaWbrA">
          <div className="huYRvB">
            <h3 className="dboodt">
              How do I place a Click &amp; Collect order?
            </h3>
            <div className="iKjAqn">
              <div className="cxTcsk">
                <img
                  src="//images.ctfassets.net/royi30b2qd26/4jo2T7BXuqB3a0DUcgKgxP/7a143e4b1a36b437e75845e29d417b9a/icons-mobile.svg"
                  alt=""
                  className="jTVAVC"
                />
                <h6 className="lehlcx">Download the Carlstons Coffee app</h6>
                <div className="ceyPlK">
                  <p>
                    Search for Carlstons Coffee in the App or Play store from
                    your mobile device and download to get started.
                  </p>
                </div>
              </div>
              <div className="cxTcsk">
                <img
                  src="//images.ctfassets.net/royi30b2qd26/68gL1dAO3bl2MHVZ3wb8ig/bb8e08d7b38d46757e26260fe48c0066/icon-card.svg"
                  alt=""
                  className="jTVAVC"
                />
                <h6 className="lehlcx">Browse our menu and place an order</h6>
                <div className="ceyPlK">
                  <p>
                    Once you’ve picked your favourite items from our menu you
                    can then place your order through the app.
                  </p>
                </div>
              </div>
              <div className="cxTcsk">
                <img
                  src="//images.ctfassets.net/royi30b2qd26/7Wg8y7nL03OBgBoap8uwd/1667bd268f32d946ef8a3a249cb96bbf/icon-location_pin.svg"
                  alt=""
                  className="jTVAVC"
                />
                <h6 className="lehlcx">
                  Select your store and collect from the counter
                </h6>
                <div className="ceyPlK">
                  <p>
                    Finally, select your nearest available store and we’ll let
                    you know when your order is ready to collect from the
                    counter.
                  </p>
                </div>
              </div>
            </div>
            {/* <div className="eHtILy">
              <span className="jkcThw linkCta">
                <a
                  href="https://swf3j.app.goo.gl/JGZwd"
                  className="link btnDark"
                >
                  Get the app
                </a>
              </span>
            </div> */}
          </div>
        </article>

        <article className="kZWJDp">
          <div className="ejxxWH">
            <p>
              *Click &amp; Collect is a service that allows Carlstons Club
              Members to pre-order and pay for selected food and drinks
              from&nbsp; participating Carlstons outlets &nbsp;before picking
              their order up at the relevant participating outlet. See
              <a href="/carlstons-coffee/costa-club/terms-of-use">
                terms and conditions
              </a>{" "}
              for more information.
            </p>
          </div>
        </article>
      </main>
    </>
  );
}

export default ClickAndCollect;
